...
**Current state of the two MVP-completion limitations, as of this section**: `Neo4jGraphBackend` is
still never run against a live server (unchanged, 11 tests still skip cleanly). The LLM limitation was
split by schema shape — event interpretation (Phase 3, flat schema) genuinely live and verified;
agent-population generation (Phase 5) still running its deterministic path because `AgentProfile`'s
schema (with its open `beliefs` dict) is incompatible with Gemini's structured output. The next section
is what changed once Phase 5 was rebuilt to work around exactly that incompatibility.

---

## Phase 5 rebuild: real Gemini-generated agent personas

The blocker above (`AgentProfile.beliefs` incompatible with Gemini's schema) is fixed by **not asking the
LLM to generate the whole profile**. A new, narrower schema — `AgentPersonaDraft` (`agent_id`, `identity`,
`strategy`, `information_sources`, `behavioral_biases`) — carries only the qualitative fields and has no
open dict, verified compatible with Gemini's structured output
(`tests/test_schema_conversion.py::test_agent_persona_draft_batch_converts_successfully_no_open_dicts`).
The numeric risk coefficients and `beliefs` are **never** generated by the LLM — they always come from
the same seeded archetype-prior sampling as before (`generate_archetype_batch_deterministic`), and
`apply_persona_overlay()` merges the LLM's qualitative fields onto that deterministic numeric skeleton.
This is the same "LLM never bypasses a deterministic risk control" principle applied to agent generation,
not just to trading.

- **Real bug found and fixed during this rebuild**: the first version derived one `RandomSeedBundle`
  component per *requested* archetype, in the order the caller's dict happened to iterate — since
  `RandomSeedBundle.derive()` assigns seeds positionally (`SeedSequence.spawn()` is order-dependent, not
  content-dependent), `{MOMENTUM: 3, CONTRARIAN: 3}` and `{CONTRARIAN: 3, MOMENTUM: 3}` produced different
  populations for the same root seed — caught by a test that compares the two orderings, not by
  inspection. Fixed by deriving from the **full, fixed, canonical list of all 12 archetype names**
  (enum definition order) every time, so an archetype's seed depends only on itself and the root seed,
  never on which other archetypes happen to be requested alongside it or in what order.
- **Optimization**: archetypes are now generated in **parallel** (`asyncio.gather`), each with its own
  independently-derived seed (a direct consequence of the fix above — independent seeding is what makes
  concurrent execution safe). Each archetype's persona batch is a real network round-trip; running the
  MVP's 10 archetypes concurrently instead of sequentially is the difference between ~10 independent
  round-trips in parallel versus their sum.
- **A second real constraint found live, which reshaped the optimization**: the configured Gemini API key
  is on the free tier, capped at a **hard 20 requests/day** for `gemini-3.5-flash`
  (`GenerateRequestsPerDayPerProjectPerModel-FreeTier`, read from the actual 429 `RESOURCE_EXHAUSTED`
  response body) — not a short burst-rate limit. A single 50-agent/10-archetype population generation
  costs up to 10 of those 20 requests. Retrying a quota-exhausted call is guaranteed to fail again and
  only wastes another request against the same daily cap, so `LLMQuotaExceededError` (a new subclass of
  `LLMUnavailableError`, detected from the response's `status: RESOURCE_EXHAUSTED` field) is now excluded
  from the retry-at-reduced-temperature logic in both `LLMBackedEventInterpreter` and
  `LLMBackedPersonaGenerator` — confirmed by tests using the exact real response body captured live, and
  a test proving exactly one call is made (not two) when this specific error occurs. A shared
  "stop-launching-more-requests" circuit breaker was considered and deliberately not built: because all
  archetype requests are dispatched together at the start of the parallel batch, by the time the first
  response returns (quota-exhausted or not) the other requests are already in flight — a breaker would
  add complexity without reducing actual request count for this call shape.
- **Real live verification, captured before the daily quota was exhausted**: a full 50-agent run produced
  10 genuine Gemini-authored personas (one per archetype) before hitting the cap on retries for the
  remainder — e.g. a real `momentum-000` persona: *"Trader retail iperattivo che passa ore davanti ai
  grafici a caccia di trend esplosivi... behavioral_biases: FOMO (Fear of Missing Out), Herding"* — the
  40 agents whose batch failed correctly fell back to the deterministic generic identity, never a crash.
  Re-verified afterward that the quota is a genuine full-day lock, not a short cooldown (a second live
  probe several minutes later still returned `RESOURCE_EXHAUSTED`).
- Tests: existing Phase 5 tests updated for the new `AgentPersonaDraft`/`LLMBackedPersonaGenerator` API
  (`AgentProfileBatch`/`LLMBackedProfileBatchGenerator` removed — no longer used by anything), plus new
  tests for the seed-independence fix, the persona overlay, and the quota-exceeded no-retry behavior in
  both call sites. 335 passing + 11 skipped (Neo4j, unchanged).
