...
- Il conto `Demo €50` (portato da €10 il 2026-08-20, stesso 25% di esposizione massima e stesso rischio per posizione — solo più capitale nominale, non un edge diverso) è salvato solo nel `localStorage` del browser: cash, frazioni, posizioni, P&L e audit non lasciano il dispositivo.
...
- L'Autopilot è opzionale e opera ogni 20 secondi con un prezzo **simulato** (o reale in modalità Live), importo massimo base €12,50 per posizione, **fino a `SIMULATION.maxConcurrentPositions` posizioni concorrenti** (20, su una watchlist di 46 simboli — non più 1:1 come in origine, il sizing reale sul capitale attuale rende ogni slot oltre il ~15-18esimo economicamente irrilevante), esposizione massima base 25% e kill switch al 20% di drawdown. La via scelta per aumentare la frequenza di trading è l'ampiezza di portafoglio (più titoli validati possono essere in posizione insieme), non una soglia di segnale più permissiva su un singolo titolo: un test walk-forward ha dimostrato che allargare la regola per generare più trade su un solo simbolo distrugge l'edge fuori campione (in-sample positivo, out-of-sample fortemente negativo).
- **Leva simulata 1:2** (`SIMULATION.leverageMultiplier`, aggiunta il 2026-08-20 su richiesta esplicita): un ordine può controllare fino a 2 volte il notional che il solo margine (cash impegnato) permetterebbe — vera contabilità a margine (`engine/execution.js`/`engine/market.js`, stile CFD/forex, mai un broker reale), non solo un tetto di sizing più alto: il cash si debita/accredita solo per il margine e per il P&L, mai per il notional pieno. La leva amplifica proporzionalmente guadagni **e perdite** sullo stesso edge misurato — non lo migliora, non lo garantisce. **"Regola d'oro" abbinata**: nessun trade, con o senza leva, può rischiare più del 5% dell'equity (`SIMULATION.maxRiskPerTradePercent`) tra prezzo di entrata e stop loss sul notional pieno — calcolato da `engine/riskGate.js` (ordini manuali) ed `engine/autopilot.js` (ingressi automatici) prima di applicare la leva, così uno stop insolitamente largo (ATR, o il minimo dell'opening range in `orb_breakout`) non può mai far superare la soglia dichiarata. Con lo stop fisso 1,6% oggi in uso questo tetto non è quasi mai vincolante — il vincolo reale resta la leva stessa.
...
- `server/mcp/server.js` — un `McpServer` reale che registra i 9 agenti come tool MCP (`technical_analyst`, `risk_manager`, `market_regime`, `liquidity`, `fundamental`, `hedge`, `audit_sentinel`, `social_sentiment`, `macro_calendar`), ciascuno con `inputSchema`/`outputSchema` Zod e lo stesso contratto strutturato di sempre (`available`, `thesis`, `confidence`, `evidence[]`, `risk_flags[]`, `model_version`).
