...
async function fetchStockTwits(symbol) {
  ...
  return {
    ...
    sample: messages.slice(0, 5).map((m) => m.body).filter(Boolean)
  };
}
...
async function fetchReddit(symbol) {
  const res = await fetch(`https://www.reddit.com/search.json?q=%24${encodeURIComponent(symbol)}&sort=new&limit=5&t=day`, {
  ...
  return {
    ...
    sample: posts.slice(0, 5).map((p) => p.data?.title).filter(Boolean)
  };
}
...
export const socialSentimentAgentTool = {
  ...
  async handler({ symbol }) {
    ...
    const parts = [];
    if (st.ok) {
      evidence.push(...st.sample.map((body) => `[StockTwits] ${body}`));
    ...
    if (rd.ok) {
      evidence.push(...rd.sample.map((title) => `[Reddit] ${title}`));
      parts.push(`Reddit: ${rd.count} post recenti`);
    }

    return {
      available: true,
      thesis: `${parts.join(' · ')} — chiacchiericcio social non filtrato per autorevolezza (nessuna fonte gratuita isola i "principali investitori"), mai un segnale quantitativo.`,
      confidence: null,
      evidence,
