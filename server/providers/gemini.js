...
export async function callGemini({ apiKey, context }) {
  ...
  const prompt = `Sei un analista che valuta titoli per un conto demo di paper trading educativo (nessun ordine reale, nessuna consulenza finanziaria). `
    ...
    + `eventuali segnalazioni di concentrazione da correlazione dall'agente Hedge, ed eventuali segnalazioni di volatilita' anomala reale (ATR) dall'agente Market Regime (se disponibili). Sii proporzionalmente piu' cauto nella confidenza quando la strategia e' esplorativa, e ancora piu' cauto quando e' una sonda (nessun edge misurato) o ci sono lezioni su fallimenti ricorrenti. Valuta se il quadro complessivo appare favorevole a una posizione long a breve termine. `
