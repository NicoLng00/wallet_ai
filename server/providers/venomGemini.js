...
// per descrivere il set di agenti reale di venom (liquidity_model, venom_news, venom_calendar al
// posto di fundamental/social_sentiment/macro_calendar) invece di lasciare un prompt che nomina
// agenti che per questa pipeline non esistono.
...
export async function callVenomGemini({ apiKey, context }) {
  ...
  const prompt = `Sei un analista che valuta azioni di club calcistici europei quotati in borsa per un conto demo di paper trading educativo (nessun ordine reale, nessuna consulenza finanziaria). `
    ...
    + `lo stato dell'agente Risk Manager (esposizione/drawdown del conto), l'agente Liquidity Model (proxy di liquidita' reale da volume di mercato: un titolo poco liquido ha spread ed esecuzione peggiori anche con un segnale tecnico corretto — mai un gate, solo un motivo per essere piu' cauti sulla dimensione), eventuali segnalazioni di concentrazione da correlazione dall'agente Hedge, eventuali segnalazioni di volatilita' anomala reale (ATR) dall'agente Market Regime, notizie pubbliche reali recenti sul club (trasferimenti, formazioni, risultati, dichiarazioni) dall'agente Venom News, ed eventuali notizie reali su calendario/prossime partite dall'agente Venom Calendar (un impegno imminente, specie in coppe europee o un derby, puo' aumentare volatilita' e volume indipendentemente dal segnale tecnico) — entrambi qualitativi, mai backtestati, mai un proxy di analisti professionali. `
    + `Questi titoli sono strutturalmente diversi da azioni ordinarie: il prezzo puo' muoversi per notizie di calciomercato, risultati sportivi, qualificazioni a competizioni internazionali — eventi che nessun indicatore tecnico cattura da solo, per questo l'evidenza qualitativa (Venom News/Calendar) ha un peso relativo maggiore che su un'azione ordinaria. Sii proporzionalmente piu' cauto nella confidenza quando la strategia e' esplorativa, e ancora piu' cauto quando e' una sonda o il titolo e' segnalato poco liquido. Valuta se il quadro complessivo appare favorevole a una posizione long a breve termine. `
